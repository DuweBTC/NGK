...
struct weather_t
{
	...
	json_io( JSON_IO & io )
	{
		io
			...
			& json_dto::mandatory( "temperature", m_humidity )
			;
	}
		...
		const restinio::request_handle_t& req, rr::route_params_t params )
	{
		...
		try
		{
			...
			for( std::size_t i = 0; i < m_weather.size(); ++i )
			{
				...
				{
					resp.append_body( std::to_string( i + 1 ) + ". " );
					resp.append_body( b.m_date + "[" + b.m_id + "]\n" );
				}
			}
		}
...
auto server_handler( weather_struct & weather_data )
{
	...
	router->http_get( "/id/:id", by( &weather_handler_t::on_id_get ) );
...
int main()
{
	...
	try
	{
		...
		weather_struct weather_data{
			...
			{ "2", "20211105", "14:15", { "Aarhus N", 13692, 19.438 }, 14.1, 65},
		};
